  ...
  for (let idx = 0; idx < chars.length; idx++) {
    ...
    charListItem.innerHTML = `
        <div class="search-item-thumbnail">
            <img src="${
              chars[idx].thumbnail.path + "." + chars[idx].thumbnail.extension
            }">
        </div>
        <div class="search-item-info"> 
            <a href = ${
              "details/charDetails.html?character=" + chars[idx].id
            }> <h3>${chars[idx].name}</h3> </a>
  ...
  searchConstList.forEach((character) => {
    ...
    character.addEventListener("click", async () => {
      ...
      const result = await fetch(
        `https://gateway.marvel.com:443/v1/public/characters/${character.dataset.id}?ts=20230223&apikey=6975c12f0f2ae6702c6d26349ef557fc&hash=0fb6598929d1b35a0704e51b09eaacdc`
      );
...
async function homeLoadHeros(query) {
  const URL =
    "https://gateway.marvel.com:443/v1/public/characters?ts=20230223&apikey=6975c12f0f2ae6702c6d26349ef557fc&hash=0fb6598929d1b35a0704e51b09eaacdc";
  const response = await fetch(`${URL}`);
  const data = await response.json();
  //  console.log(data.data.results);
...
function displayLoadHeros(data) {
  for (let i = 0; i < data.length; i++) {
    ...
    charListItem.innerHTML = `<li id="home-single-hero">
